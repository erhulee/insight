...
export default function SelfRenderPage() {
	const [activeTab, setActiveTab] = useState<'upload' | 'url' | 'editor'>(
		'upload',
	)
	...
	const validateSpec = (content: string) => {
		...
		try {
			...
			if (!hasSurveyEndpoints) {
				errors.push(
					'规范中没有问卷相关的端点。自渲染功能需要问卷相关的API端点。',
				)
			}
						...
						<div key={question.id} className="p-4 border rounded-lg">
							<div className="flex items-start gap-2 mb-2">
								<h3 className="text-lg font-medium">
									{renderedSurvey.settings?.showQuestionNumbers &&
										`${index + 1}. `}
									{question.title}
									{question.required && (
