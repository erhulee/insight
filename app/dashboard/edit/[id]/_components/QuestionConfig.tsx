...
export function QuestionConfig({ onUpdate }: QuestionConfigProps) {
  ...
  const selectQuestion = runtimeState.currentQuestion.find(
    (item) => item.field == runtimeState.selectedQuestionID,
  )!
  const questionInfo = useMemo(
    () => preset.find((item) => item.type === selectQuestion.type)!,
    [selectQuestion.type],
  )
    ...
    <div className="space-y-6">
      <Tabs defaultValue="basic" className="w-full">
        <TabsList className="w-full grid grid-cols-3">
          ...
          <TabsTrigger
            value="options"
            disabled={!['radio', 'checkbox', 'dropdown'].includes(selectQuestion.type)}
          >
            选项
          </TabsTrigger>
