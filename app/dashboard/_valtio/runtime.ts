...
function updateCurrentQuestion() {
    runtimeStore.currentQuestion = runtimeStore.questions.filter(q => q.ownerPage == runtimeStore.currentPage)
    if (runtimeStore.currentQuestion.findIndex(q => q.field === runtimeStore.selectedQuestionID) == -1) {
...
export const deletePage = (pageIndex: number) => {
    ...
    runtimeStore.questions = runtimeStore.questions.filter(q => q.ownerPage !== pageIndex).map((q) => {
        return {
            ...q,
