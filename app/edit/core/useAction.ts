...
import {
	preset,
	QuestionType,
} from '@/components/survey-editor/buildin/form-item'
import {
	addQuestion,
	deleteQuestion,
	selectQuestion,
...
import type { Question } from '@/app/(dashboard)/dashboard/_valtio/runtime'
...
export const useAction = () => {
	 ...
	 * Select a question
	 */
	const handleSelectQuestion = useCallback((question: QuestionSchemaType) => {
	...
	const handleAddQuestion = useCallback((type: QuestionType) => {
		...
		if (!meta) return
		const id = uuidv4()
		const newQuestion: QuestionSchemaType = QuestionSchema.parse({
			id: id,
	...
	const handleDragEnd = useCallback(
		(event: DragEndEvent) => {
			...
			const fromQuestionIndex = questions.findIndex(
				(q) => q.id === fromQuestionId,
			)
	 ...
	 */
	const batchUpdateQuestions = useCallback(
		(updates: Array<{ id: string; updates: Partial<QuestionSchemaType> }>) => {
