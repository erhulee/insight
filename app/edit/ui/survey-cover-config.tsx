...
export function SurveyCoverConfig({
	coverConfig,
	onUpdate,
}: SurveyCoverConfigProps) {
		...
		<div className="flex-1 px-4 overflow-y-auto h-full">
			<div className="space-y-4 py-2">
				...
				<div className="space-y-2">
					...
					<div className="space-y-1.5">
						<Label htmlFor="coverDescription" className="text-xs">
							封面描述
						</Label>
						<Textarea
							id="coverDescription"
				...
				<div className="space-y-3">
					<h3 className="text-sm font-medium text-foreground">隐私和说明</h3>
					<div className="space-y-1.5">
						<Label htmlFor="privacyNotice" className="text-xs">
							隐私保护说明
						</Label>
						<Textarea
							id="privacyNotice"
					...
					<div className="space-y-1.5">
						<Label htmlFor="bottomNotice" className="text-xs">
							底部说明文案
						</Label>
						<Textarea
							id="bottomNotice"
				...
				{/* 视觉样式 */}
				<div className="space-y-3">
					<h3 className="text-sm font-medium text-foreground">视觉样式</h3>

					<div className="grid grid-cols-2 gap-3">
						<div className="space-y-1.5">
							<Label htmlFor="coverIcon" className="text-xs">
								封面图标
							</Label>
							<Select
						...
						<div className="space-y-1.5">
							<Label htmlFor="coverColor" className="text-xs">
								主色调
							</Label>
							<Select
				...
				{/* 显示控制 */}
				<div className="space-y-3">
					<h3 className="text-sm font-medium text-foreground">显示控制</h3>
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(checked) =>
									handleChange('showProgressInfo', checked)
								}
								className="scale-75"
							/>
						</div>
						...
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(checked) =>
									handleChange('showPrivacyNotice', checked)
								}
								className="scale-75"
							/>
						</div>
				...
				<div className="space-y-2">
					...
					<div className="bg-muted rounded-lg p-3 border border-border">
						<div className="text-center">
							<div
								className={`w-12 h-12 ${COLOR_OPTIONS.find((c) => c.value === (coverConfig.coverColor || 'blue'))?.color || 'bg-blue-500'} rounded-lg flex items-center justify-center mx-auto mb-2`}
							>
								{(() => {
									const IconComponent =
										ICON_OPTIONS.find(
											(i) => i.value === (coverConfig.coverIcon || 'file-text'),
										)?.icon || FileText
									return <IconComponent className="h-6 w-6 text-white" />
								})()}
							</div>
							<p className="text-xs text-muted-foreground mb-1">
								{coverConfig.estimatedTime || '3-5分钟'} •{' '}
								{coverConfig.showProgressInfo !== false
									? '显示进度信息'
									: '隐藏进度信息'}
							</p>
