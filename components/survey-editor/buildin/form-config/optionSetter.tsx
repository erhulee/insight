    ...
    <div id={id}>
      <div className="flex flex-col gap-1 items-center w-full">
        {value.map((item: string, index: number) => (
          <div key={item + index} className=" w-full flex flex-row">
            <Input
              value={value[index]}
              className=" flex-1 focus-visible:shadow-none mr-2"
              onChange={(e) => {
        ...
        <div className=" w-full flex flex-row">
          ...
          <Button
            ...
            variant="ghost"
            className=" w-8 h-8"
            disabled={additionContent == '' || value.includes(additionContent)}
            onClick={() => {
              if (additionContent !== '') {
