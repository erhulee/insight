...
import { MultipleQuestionSchemaType } from '@/lib/dsl'
import { Radio } from 'antd'
import { PlusCircle } from 'lucide-react'
import { useRef, useState } from 'react'

function MultipleQuestionItem(props: { label: string }) {
...
export function MultipleQuestion({ dsl }: { dsl: MultipleQuestionSchemaType }) {
  const { props } = dsl
  const { options } = props!
  ...
  return (
    <div className=" flex flex-col gap-1">
      {options.map((opt) => (
        <MultipleQuestionItem label={opt.label}></MultipleQuestionItem>
      ))}
      <div>
